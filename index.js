  ...
  try {
    ...
    app.get("/admin-stats", verifyJWT, verifyAdmin, async (req, res) => {
      ...
      */
      ...
      const revenueCal = payments.reduce(
        (sum, payment) => sum + payment.price,
        0
      );
      const revenue = parseFloat(revenueCal.toFixed(2));
     ...
     * 5. separate aiiOrderedItems by category using filter
     * 6. now get the quantity by using length: pizza.length
     * 7. for each category use the reduce to get the total amount spent on this category
     *
     */
